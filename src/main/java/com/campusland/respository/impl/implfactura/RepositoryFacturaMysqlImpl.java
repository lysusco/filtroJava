...
public class RepositoryFacturaMysqlImpl implements RepositoryFactura {
    ...
    private static final String SQL_GET_LIST_IMPORTE_TOTAL = "SELECT SUM(importe) from item_factura";
    ...
    @Override
    public List<Factura> listarVentasTotales() {
        List<Factura> listFacturas = new ArrayList<>();

        try (Connection conn = getConnection();
                PreparedStatement preparedStatement = conn.prepareStatement(SQL_GET_LIST_IMPORTE_TOTAL);
                ResultSet rs = preparedStatement.executeQuery()) {
